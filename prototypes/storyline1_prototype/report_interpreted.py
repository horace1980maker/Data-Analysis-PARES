...
def section_capacity(tables: Dict[str, pd.DataFrame]) -> str:
    ...
    # Detect data source for methodology blurb
    capacity_tbl = tables.get("capacity_overall_by_mdv", pd.DataFrame())
    cap_questions = tables.get("capacity_overall_questions", pd.DataFrame())
    is_aggregated = (
        not cap_questions.empty
        and "question_id" in cap_questions.columns
        and cap_questions["question_id"].astype(str).str.contains("indicador|indicator", case=False, na=False).any()
    ) or (
        not capacity_tbl.empty and capacity_tbl.get("n_responses", pd.Series()).max() < 50
        and tables.get("capacity_by_group_by_mdv", pd.DataFrame()).empty
    )

    if is_aggregated:
        parts.append(_interp(
            "Los puntajes de capacidad adaptativa provienen de datos agregados (<strong>TIDY_7_1_CAPACITY</strong>). "
            "Se registraron puntajes promedio por indicador y medio de vida en una escala de 1 a 5, "
            "los cuales se normalizan a una escala de 0 a 1: "
            "<strong>Capacidad = (puntaje − 1) / 4</strong>, y <strong>Brecha = 1 − Capacidad</strong>. "
...
def generate_interpreted_report(input_dir: str) -> str:
    """
    Generate the interpreted HTML report from a Storyline 1 output directory.

    Args:
        input_dir: Path to storyline1 output dir (must contain tables/ and figures/)
